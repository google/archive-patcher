...
public class PatchMagic {
     ...
     * this library.
     */
    private final static ArchivePatcherVersion BUILD_VERSION =
        ArchivePatcherVersion.V2;
    ...
    public static String getStandardHeader() {
        return MAGIC + BUILD_VERSION.asTwoDigitString;
    }
     ...
     * {@link #readStandardHeader(DataInput)}, extracts the version from that
     * that header and returns it.
     * @param header
     * @return the version
     */
    public static ArchivePatcherVersion getPatchVersion(String header) {
        if (!header.matches(MAGIC_REGEX)) throw new IllegalArgumentException(
                "Bad header: " + header);
        return ArchivePatcherVersion.parse(header.substring(MAGIC.length()));
    }
}
