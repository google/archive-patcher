...
/**
 * Defines the default deflate compatibility window.
 */
public class DefaultDeflateCompatibilityWindow {
  ...
  // TODO: Skip the zlib header bytes for fingerprint computation and simply
  // ignore the "wrapped" set of values completely. This will cut fingerprints
  // by half and fix compatibility issues with older JVMs caused by this zlib
  // commit that twiddled the level flag assignments in the zlib header fields:
  ...
  /**
   * Base of corpus for compression fingerprinting.
   */
  private final static String CORPUS_BASE_TEXT =
  ...
  /**
   * Runs the check and returns 0 if compatible, otherwise 1.
   */
  public final static void main(String... args) throws Exception {
    StringBuilder buffer = new StringBuilder();
    if (!DefaultDeflateCompatibilityWindow.isCompatible(buffer)) {
   ...
   * @return the corpus
   */
  public static final byte[] generateCorpus() {
   ...
   */
  public static final boolean isCompatible(StringBuilder outBuffer) {
    try {
      ...
      if (outBuffer != null) {
        for (JreDeflateParameters parameters : baseline.keySet()) {
          ...
          final boolean matches = baselineSHA256.equals(computedSHA256);
          outBuffer
              .append(matches ? "OK   " : "FAIL ")
  ...
  /**
   * Emit baseline information as a string.
   * @throws IOException if anything goes wrong
   */
  public static final String getBaseline() throws IOException {
    ...
    while (iterator.hasNext()) {
      ...
      buffer.append(resultToString(oneResult.getKey(), oneResult.getValue()));
      buffer.append("\n");
    }
    return buffer.toString();
  }
  ...
  /**
   * Emit source code for a new baseline as a string.
   * @throws IOException if anything goes wrong
   */
  public static final String emitBaselineCode() throws IOException {
    StringBuilder buffer = new StringBuilder();
   ...
   */
  private static final Map<JreDeflateParameters, String> compute() throws IOException {
    final boolean[] nowrapValues = {true, false};
    final int[] strategies = {Deflater.DEFAULT_STRATEGY, Deflater.FILTERED, Deflater.HUFFMAN_ONLY};
    ...
    for (final boolean nowrap : nowrapValues) {
      for (final int strategy : strategies) {
        ...
        for (final int level : relevantLevels) {
          ...
          try {
            sha256OfCompressedData = MessageDigest.getInstance("SHA-256").digest(compressedData);
   ...
   */
  private static final String resultToString(
   ...
   * @return the mapping
   */
  private static final Map<JreDeflateParameters, String> readBaseline() {
    Map<JreDeflateParameters, String> result = new LinkedHashMap<JreDeflateParameters, String>();
    String[] lines = BASELINE.split("\n");
    for (String line : lines) {
      JreDeflateParameters parameters = JreDeflateParameters.parseString(line);
