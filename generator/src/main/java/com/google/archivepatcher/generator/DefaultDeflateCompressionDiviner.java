...
public class DefaultDeflateCompressionDiviner {
  ...
  private static boolean matches(
    ...
    // We have to make sure we can open two streams.
    ByteSource compressedDataCopy;
    if (compressedData.supportsMultipleStreams()) {
      compressedDataCopy = compressedData;
    } else {
      compressedDataCopy = compressedData.copy();
    }
    ...
    try (MatchingOutputStream matcher =
        ...
        InflaterInputStream inflaterIn =
            new InflaterInputStream(compressedDataCopy.openStream(), inflater, copyBuffer.length);
